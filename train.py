...
def train_mlm(config):
    ...
    data_mlm = list(pd.read_csv(C.DATA_DIR+config.mlm_name)['generate'])
    ...
    model_mlm.save_adapter(save_directory=C.ADAPTER_DIR, adapter_name = 'mlm_adapter', with_head = True)
...
def train_cls(config):
    ...
    if config.activate_adapter_cls == True:
        model_cls.load_adapter(adapter_name_or_path=C.ADAPTER_DIR, set_active = True)
    ...
    data_train_cls  = pd.read_csv(C.DATA_DIR+C.TRAIN_CLS, index_col=0)
